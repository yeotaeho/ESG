...
public class NaverController {
    ...
    public ResponseEntity<Map<String, Object>> naverCallback(@RequestBody Map<String, String> body) {
        ...
        try {
            ...
            User user = userService.findOrCreateUser("naver", providerId, email, name, nickname, profileImage);

            // 2. JWT 토큰 생성
            String accessToken = jwtTokenUtil.generateToken(user.getId(), "naver", email);
            String refreshToken = jwtTokenUtil.generateRefreshToken(user.getId(), "naver", email);
