...
const RECENT_ARTICLES = [
  { type: '보도자료', title: '삼성전자, 대규모 미디어데이 행사서 새로운 비전 공개', date: '2025.12.04', image: 'https://placehold.co/400x250/000000/FFFFFF?text=NEWS' },
  { type: '영상', title: '[영상] "폴더블폰 조립은 이렇게..." 갤럭시 Z 트라이폴드 제작 과정', date: '2025.12.04', image: 'https://placehold.co/400x250/000000/FFFFFF?text=VIDEO' },
  { type: '보도자료', title: '[영상] 밸런타인 홈 오디오의 실체는? 프리미엄 스피커의 비밀', date: '2025.12.04', image: 'https://placehold.co/400x250/000000/FFFFFF?text=AUDIO' },
  { type: '보도자료', title: '[CES 2026] 삼성전자 "디지털 트랜스포메이션" 초대장', date: '2025.12.04', image: 'https://placehold.co/400x250/000000/FFFFFF?text=CES' },
];
const MORE_STORIES = [
...
const ArticleCard = ({ article, large = false }: ArticleCardProps) => (
...
const SectionHeader = ({ title, showPagination = false, hasButton = false }: SectionHeaderProps) => (
...
export default function App() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-12">
          ...
          <nav className="hidden lg:flex space-x-6 text-sm">
            {NAV_LINKS.map(link => (
              <a key={link} href="#" className="text-gray-600 hover:text-red-600 transition font-medium">
                {link}
              </a>
            ))}
          </nav>
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
      ...
      <div className="flex overflow-x-auto space-x-4 pb-2 border-b border-gray-200 mb-8 whitespace-nowrap">
        {HEADER_LINKS.map(link => (
          <button
            ...
            className={`py-2 px-3 text-sm font-medium transition duration-300 ease-in-out ${activeTab === link
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {RECENT_ARTICLES.map((article, index) => (
          <div key={index} className="flex flex-col group rounded-lg overflow-hidden hover:shadow-lg transition">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {MORE_STORIES.map((article, index) => (
  ...
  const HighlightSection = ({ title, items }: HighlightSectionProps) => (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
      <SectionHeader title={title} showPagination={true} />
      <div className="relative">
        <div className="flex overflow-x-scroll snap-x snap-mandatory space-x-4 scrollbar-hide">
          {items.map((item: HighlightItem, index: number) => (
            <div key={index} className="flex-shrink-0 w-64 sm:w-80 snap-center rounded-lg overflow-hidden shadow-lg group">
              ...
              <div className="p-4 bg-white">
                <h3 className="text-lg font-bold group-hover:text-red-600 transition">{item.title}</h3>
                {item.subtitle && <p className="text-sm text-gray-500 mt-1">{item.subtitle}</p>}
              </div>
            </div>
          ))}
          {/* Placeholder for scroll-snap look on desktop */}
          <div className="w-1/2 flex-shrink-0 sm:w-1/3"></div>
        </div>
        {/* Arrow Navigation (Desktop Only) */}
        <div className="hidden lg:flex absolute top-1/2 -translate-y-1/2 -left-12 -right-12 justify-between w-full pointer-events-none">
          <button className="p-3 bg-white border rounded-full shadow-md text-gray-700 hover:bg-gray-100 transition pointer-events-auto"><ChevronLeft size={20} /></button>
          <button className="p-3 bg-white border rounded-full shadow-md text-gray-700 hover:bg-gray-100 transition pointer-events-auto"><ChevronRight size={20} /></button>
        </div>
      </div>
      {/* style jsx 태그를 표준 style 태그로 변경하여 경고 해결 */}
      <style>{`
  ...
  return (
    <div className="min-h-screen font-sans bg-white">
      ...
      <main>
        ...
        <NewsroomSections />
        {/* 확인해 보세요 */}
        <HighlightSection title="확인해 보세요" items={DISCOVERY_ITEMS} />
        {/* 미디어 하이라이트 */}
        <HighlightSection title="미디어 하이라이트" items={MEDIA_HIGHLIGHTS} />
      </main>
      <Footer />
    </div>
  );
